...
fn share_refresh_works<E>(n: u16, reliable_broadcast: bool)
where
    ...
    Point<E>: generic_ec::coords::HasAffineX<E>,
{
    ...
    let refreshed = round_based::sim::run(n, |i, party| {
        let party = cggmp24_tests::buffer_outgoing(party);
        let mut party_rng = rng.fork();
        let core = &incomplete_shares[usize::from(i)];
        async move {
            cggmp24::share_refresh::<E, E::SecurityLevel>(eid, i, n, core)
                .set_digest::<E::Digest>()
                .enforce_reliable_broadcast(reliable_broadcast)
                .start(&mut party_rng, party)
                .await
    ...
    for output in &refreshed {
        assert_eq!(output.core.shared_public_key, original_pk);
    }
    ...
    let refreshed_key_shares: Vec<KeyShare<E, E::SecurityLevel>> = refreshed
        ...
        .zip(aux_infos)
        .map(|(output, aux)| {
            KeyShare::from_parts((output.core, aux)).expect("valid refreshed key share")
        })
        .collect();
