...
\begin{itemize}
    \item $\kappa$ determines the target security bits of the protocol, it should match the security level of the used elliptic curve.

    \item $\kappa_\rsa$ is a bit-length of the primes used for Paillier private keys. It's chosen to match $\kappa$ bits of security.
    \item $n_\rsa$ is minimal length of the Paillier public key, private key of which has $\kappa_\rsa$ bits.
...
\algoInputs{security level $L = (\kappa_\rsa, \dots)$}
\algoOutputs{Pedersen public parameters $N, s, t \in \Z^3$, and secret parameter $\lambda \in \Z$}
\begin{algorithmic}[1]
...
\begin{description}
    ...
    \begin{algorithmic}[1]
        ...
        \State Set $\psi_i = \proveni{elog}(\Encode(\sid, i), (B_{i,1}, B_{i,2}, Y_i, \Gamma_i, G); (\gamma_i, b_i))$
        \For{$j \in [n] \setminus \{i\}$}
            ...
            \State Send $(\Gamma_i, \psi_i, D_{j,i}, F_{j,i}, \hat{D}_{j,i}, \hat{F}_{j,i}, \psi_{j,i}, \hat{\psi}_{j,i})$ to $P_j$
        \EndFor
    \end{algorithmic}

    \item[\textbf{Round 3.}] \ 
    \begin{algorithmic}[1]
        \State Receive $(\Gamma_j, \psi_i, D_{i,j}, F_{i,j}, \hat{D}_{i,j}, \hat{F}_{i,j}, \psi_{i,j}, \hat{\psi}_{i,j})
            \in (\E, *, \Z, \Z, \Z, \Z, *, *)$ from all parties

            \Comment{domains of the proofs are specified within ZK proofs verify functions}

        \For{$j \in [n] \setminus \{i\}$}
            \State Assert $\verifyni{elog}(\Encode(\sid, j), (B_{j,1}, B_{j,2}, Y_j, \Gamma_j, G), \psi_j)$
