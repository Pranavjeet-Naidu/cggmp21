...
Key refresh re-randomizes additive secret shares without changing the joint public key. It is the share-refresh protocol of~\cite[Figure~7]{cggmp24} with auxiliary-information generation removed (that is performed by Section~\ref{section:aux_data_gen}). Parties may run it periodically for proactive security; each successful execution outputs a new epoch identifier~$\rid$, an updated secret share~$x^*_i$, and updated public key shares~$\vec X^* = (X^*_k)_{k\in[n]}$.
...
Implementations should enforce this at the application or orchestration layer.
...
\begin{description}
    ...
    \begin{algorithmic}[1]
        ...
        \State Set $x'_{i,n-1} = -\sum_{j=0}^{n-2} x'_{i,j} \bmod q$
               \Comment{\parbox[t]{.45\linewidth}{implements~\cite[Fig.~7 step~(d)]{cggmp24}: $\sum_{k \in [n]} x'_{i,k} = 0$}}
    ...
    \begin{algorithmic}[1]
        ...
        \For{$j \in [n] \setminus \{i\}$}
            ...
            \State Assert $\sum_{k \in [n]} X'_{j,k} \? = \mathcal{O}$
                   \Comment{\parbox[t]{.45\linewidth}{$\mathcal{O}$ is the identity of $\E$; checks that $P_j$'s contributions sum to zero}}
        \EndFor
        \State Compute $\rid = \bigoplus_{j \in [n]} \rid_j$
        \For{$j \in [n] \setminus \{i\}$}
            \State Compute $\rho_{i,j} \in \Z_q = \hashtoscalar\bigl(\Encode[refresh\_mask](\sid, \rid, i, y_{i,j} \cdot Y_{j,i})\bigr)$
    ...
    \begin{algorithmic}[1]
        ...
        \For{$j \in [n] \setminus \{i\}$}
            \State Compute $\rho_{j,i} \in \Z_q = \hashtoscalar\bigl(\Encode[refresh\_mask](\sid, \rid, j, y_{j,i} \cdot Y_{i,j})\bigr)$
        ...
        \State Set $x^*_i = x_i + \sum_{j \in [n]} x'_{j,i} \bmod q$
               \Comment{the $j = i$ term $x'_{i,i}$ is known locally}
